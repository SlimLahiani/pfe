...
const badgeVariants: Record<BadgeVariant, string> = {
  default: 'bg-black/5 text-gray-700 border-black/10',
...
export const StatCard: React.FC<StatCardProps> = ({
  label, value, icon, trend, trendUp, colorClass = 'bg-primary/10 text-primary'
}) => (
    ...
    <div className="flex justify-between items-start mb-4">
      ...
      {trend && (
        <span className={cn(
          ...
          trendUp ? 'text-emerald-700 bg-emerald-500/10' : 'text-red-700 bg-red-500/10'
        )}>
          {trend}
        </span>
      )}
            ...
            {isLoading ? (
                ...
                <tr key={i} className="animate-pulse">
                  {columns.map((col) => (
                    <td key={col.key} className="px-5 py-4">
                      <div className="h-4 bg-black/5 rounded-lg w-3/4" />
                    </td>
                  ))}
                </tr>
              ))
              ...
              data.map((row) => (
                <tr
                  ...
                  className={cn(
                    'text-sm text-foreground hover:bg-black/[0.02] transition-colors',
    ...
    <div className="flex items-center justify-between px-1">
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          className="px-3 py-1.5 rounded-lg text-xs font-medium bg-black/5 text-gray-700 border border-black/5 disabled:opacity-30 hover:bg-black/10 transition-colors"
        >
          ← Prev
        </button>
          ...
          <button
            ...
            className={cn(
              ...
              p === page
                ...
                : 'bg-black/5 text-gray-700 border-black/5 hover:bg-black/10'
            )}
          >
            {p}
          </button>
...
const buttonVariants: Record<ButtonVariant, string> = {
  ...
  secondary: 'bg-black/5 hover:bg-black/10 text-foreground border border-black/10',
  danger: 'bg-red-500/10 hover:bg-red-500/20 text-red-700 border border-red-500/20',
  ghost: 'hover:bg-black/5 text-gray-500 hover:text-foreground',
};
...
export const Tabs: React.FC<TabsProps> = ({ tabs, active, onChange }) => (
  <div className="flex gap-1 bg-black/[0.03] border border-black/5 rounded-xl p-1">
    {tabs.map((tab) => (
      <button
        key={tab.value}
